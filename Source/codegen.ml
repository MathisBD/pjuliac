...
let build_frame frame_size =
  repeat frame_size (pushq (imm uninit_value))

let destroy_frame frame_size =
...
and compile_expr te = match te.expr with
  ...
  | TEaccess_var v ->
    begin match v with
      | StackLocal sl ->
        movq (ind ~ofs:(-8 - 8 * sl.index) rbp) !%r8 ++
        cmpq (imm uninit_value) !%r8 ++ jne l1 ++
        error (Printf.sprintf "unitinialized variable %s" sl.name) ++
        label l1 ++
        pushq !%r8
  ...
  | TEassign_var (v, te1) ->
    ...
    begin match v with
      | StackLocal sl ->
        movq !%r8 (ind ~ofs:(-8 - 8 * sl.index) rbp)
      | _ -> failwith "not implemented"
    end
  | _ -> failwith "not implemented"
...
let compile_decl = function
  | TDexpr (te, frame_size) -> 
    let code = 
      build_frame frame_size ++
      compile_expr te ++
      incq !%rsp ++
      destroy_frame frame_size
    in
